...
#include <vector>  // For std::vector (NEW)
...
// NEW: Global vectors to store UdpClient application pointers for each slice
std::vector<Ptr<Application>> g_urllcClientApps;
std::vector<Ptr<Application>> g_embbClientApps;
std::vector<Ptr<Application>> g_mmtcClientApps;

// NEW: Global for ML control file name
std::string g_mlControlFileName = "ml_traffic_control.txt"; // This file will be read by C++

// Forward declaration of the new function for control file
void ReadMlControlFileAndApplyParameters();
...
// NEW FUNCTION: Reads ML control file and applies parameters
void
ReadMlControlFileAndApplyParameters()
{
    NS_LOG_INFO("--- Reading ML control file at time: " << Simulator::Now().GetSeconds() << " s ---");

    std::ifstream inputFile(g_mlControlFileName);
    if (!inputFile.is_open())
    {
        NS_LOG_WARN("ML control file '" << g_mlControlFileName << "' not found or could not be opened. Using current parameters. Will retry.");
        // Reschedule even if file not found, perhaps it will be created by Python later
        Simulator::Schedule(Seconds(10.0), &ReadMlControlFileAndApplyParameters);
        return;
    }
    ...
    while (std::getline(inputFile, line))
    {
        ...
        if (eqPos != std::string::npos)
        {
            ...
            // Trim whitespace from key if necessary
            ...
            try
            {
                params[key] = std::stoul(valueStr); // Convert string to unsigned long (uint32_t)
            }
            catch (const std::exception& e)
            {
                NS_LOG_WARN("Error parsing value for key '" << key << "': " << e.what() << ". Line: '" << line << "'");
            }
        }
    }
    inputFile.close();

    // Apply parameters to URLLC clients
    if (params.count("urllc_packet_size") && params.count("urllc_interval_us"))
    {
        uint32_t packetSize = params["urllc_packet_size"];
        uint32_t intervalUs = params["urllc_interval_us"];
        for (auto appPtr : g_urllcClientApps)
        {
            Ptr<UdpClient> client = appPtr->GetObject<UdpClient>(); // Cast to UdpClient type
            if (client) // Ensure the cast was successful
            {
                client->SetAttribute("PacketSize", UintegerValue(packetSize));
                client->SetAttribute("Interval", TimeValue(MicroSeconds(intervalUs)));
            }
        }
        NS_LOG_INFO("  URLLC: PacketSize=" << packetSize << ", Interval=" << intervalUs << " us");
    }
    else
    {
        NS_LOG_WARN("  URLLC parameters not found in control file. Keeping current values.");
    }

    // Apply parameters to eMBB clients
    if (params.count("embb_packet_size") && params.count("embb_interval_us"))
    {
        uint32_t packetSize = params["embb_packet_size"];
        uint32_t intervalUs = params["embb_interval_us"];
        for (auto appPtr : g_embbClientApps)
        {
            Ptr<UdpClient> client = appPtr->GetObject<UdpClient>();
            if (client)
            {
                client->SetAttribute("PacketSize", UintegerValue(packetSize));
                client->SetAttribute("Interval", TimeValue(MicroSeconds(intervalUs)));
        ...
        NS_LOG_INFO("  eMBB: PacketSize=" << packetSize << ", Interval=" << intervalUs << " us");
    }
    else
    {
        NS_LOG_WARN("  eMBB parameters not found in control file. Keeping current values.");
    }

    // Apply parameters to mMTC clients
    if (params.count("mmtc_packet_size") && params.count("mmtc_interval_us"))
    {
        uint32_t packetSize = params["mmtc_packet_size"];
        uint32_t intervalUs = params["mmtc_interval_us"];
        for (auto appPtr : g_mmtcClientApps)
        {
            Ptr<UdpClient> client = appPtr->GetObject<UdpClient>();
            if (client)
            {
        ...
        NS_LOG_INFO("  mMTC: PacketSize=" << packetSize << ", Interval=" << intervalUs << " us");
    }
    else
    {
        NS_LOG_WARN("  mMTC parameters not found in control file. Keeping current values.");
    }

    // Reschedule this function to be called again after 10 seconds
    Simulator::Schedule(Seconds(10.0), &ReadMlControlFileAndApplyParameters);
}
...
main (int argc, char *argv[])
{
  ...
  // NEW: Enable logging for UdpClient to see attribute changes
  ...
  // NEW: Add command line argument for the ML control file
  cmd.AddValue ("mlControlFile", "Filename for ML traffic control parameters (read by C++, written by Python)", g_mlControlFileName);
  cmd.Parse (argc, argv);
  ...
  uint32_t ues = 3; // UEs per mmWave ENB node (original logic implies 3 UEs per mmWave ENB)
  ...
  // ApplicationContainer clientApp; // This will no longer be used collectively
  ...
  for (uint32_t u_idx = 0; u_idx < numUePerSlice; ++u_idx)
    {
      ...
      UdpClientHelper dlClient (remoteHostAddr, portUdp);
      // Initial values set here. These will be overridden by the control file if present.
      dlClient.SetAttribute ("Interval", TimeValue (MicroSeconds (200)));
      dlClient.SetAttribute ("MaxPackets", UintegerValue (UINT32_MAX));
      dlClient.SetAttribute ("PacketSize", UintegerValue (60));
      
      ApplicationContainer installedClients = dlClient.Install (remoteHost);
      g_urllcClientApps.push_back(installedClients.Get(0)); // Store Ptr to the application

      installedClients.Start (MilliSeconds (100));
      installedClients.Stop (Seconds (simTime - 0.1));
  ...
  for (uint32_t u_idx = numUePerSlice; u_idx < 2 * numUePerSlice; ++u_idx)
    {
      ...
      dlClient.SetAttribute ("MaxPackets", UintegerValue (UINT32_MAX));
      dlClient.SetAttribute ("PacketSize", UintegerValue (5000));
      
      ApplicationContainer installedClients = dlClient.Install (remoteHost);
      g_embbClientApps.push_back(installedClients.Get(0)); // Store Ptr to the application
  ...
  for (uint32_t u_idx = 2 * numUePerSlice; u_idx < nUeNodes; ++u_idx)
    {
      ...
      UdpClientHelper dlClient (remoteHostAddr, portUdp);
      dlClient.SetAttribute ("Interval", TimeValue (MilliSeconds (10000)));
      dlClient.SetAttribute ("MaxPackets", UintegerValue (UINT32_MAX));
      dlClient.SetAttribute ("PacketSize", UintegerValue (16));
      
      ApplicationContainer installedClients = dlClient.Install (remoteHost);
      g_mmtcClientApps.push_back(installedClients.Get(0)); // Store Ptr to the application
  ...
  // Start PacketSink applications
  remoteHostSinkApp.Start (Seconds (0));
  ueSinkApp.Start (Seconds (0));
  
  // NOTE: clientApp.Start/Stop below are removed if you're starting/stopping individually above.
  // The original C++ had this:
  // clientApp.Start (MilliSeconds (100));
  // clientApp.Stop (Seconds (simTime - 0.1));

  // NEW: Schedule the initial call to read the ML control file
  // Schedule it slightly after applications start to give them time to initialize
  Simulator::Schedule (Seconds (0.2), &ReadMlControlFileAndApplyParameters);
