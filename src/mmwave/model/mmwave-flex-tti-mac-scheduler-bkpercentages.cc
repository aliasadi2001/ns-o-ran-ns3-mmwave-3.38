    ...
    const struct MmWaveMacSchedSapProvider::SchedTriggerReqParameters& params)
{
    ...
    if (nFlowsTot > 0)
    {
        ...
        while (remSym > 0 && allocated)
        {
            ...
            {
                nRemSymPerFlow = 1;
                std::cout << "[1328] nFlowDL -> " << nFlowsDl << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0 << " nRemSymPerFlow (remSym / nFlowsTot) -> " << nRemSymPerFlow<< std::endl; 
            ...
            if (m_fixedTti)
            {
                nRemSymPerFlow = ceil((double)nRemSymPerFlow / (double)m_symPerSlot) *
                                 m_symPerSlot; // round up to nearest sym per TTI
                std::cout << "[1335] nFlowDL -> " << nFlowsDl << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0 << " nRemSymPerFlow (remSym / nFlowsTot) -> " << nRemSymPerFlow<< std::endl; 
            }
            while (remSym > 0)
            {
                int addSym = 0;
                ...
                NS_ASSERT(deficit >= 0);
                std::cout << "[1343] nFlowDL -> " << nFlowsDl << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0
                ...
                if (m_fixedTti)
                {
                    deficit = ceil((double)deficit / (double)m_symPerSlot) *
                              m_symPerSlot; // round up to nearest sym per TTI
                    std::cout << "[1351] nFlowDL -> " << nFlowsDl << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0
                             << " nRemSymPerFlow (remSym / nFlowsTot) -> " << nRemSymPerFlow<<
                             "addSym = "<< addSym << "deficit = "<< deficit << std::endl;
                }
                ...
                {
                    std::cout << "[1358] nFlowDL -> " << nFlowsDl << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0
                             << " nRemSymPerFlow (remSym / nFlowsTot) -> " << nRemSymPerFlow<<
                    ...
                    if (deficit < nRemSymPerFlow)
                    {
                        ...
                        {
                            addSym = remSym;
                            std::cout << "[1367] nFlowDL -> " << nFlowsDl << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0
                                     << " nRemSymPerFlow (remSym / nFlowsTot) -> " << nRemSymPerFlow<<
                                    "addSym = "<< addSym << "deficit = "<< deficit << std::endl;
                        }
                        else
                        {
                            ...
                            std::cout << "[1379] nFlowDL -> " << nFlowsDl<<"nFlowtot -> " << nFlowsTot << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0
                                 << " nRemSymPerFlow (remSym / nFlowsTot) -> " << nRemSymPerFlow<<
                                "addSym = "<< addSym << "deficit = "<< deficit<<
                    ...
                    else
                    {
                        ...
                        {
                            addSym = remSym;
                            std::cout << "[1390] nFlowDL -> " << nFlowsDl << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0
                                 << " nRemSymPerFlow (remSym / nFlowsTot) -> " << nRemSymPerFlow<<
                                "addSym = "<< addSym << "deficit = "<< deficit << std::endl;
                        }
                        else
                        {
                            ...
                            std::cout << "[1397] nFlowDL -> " << nFlowsDl << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0
                                 << " nRemSymPerFlow (remSym / nFlowsTot) -> " << nRemSymPerFlow<<
                                "addSym = "<< addSym << "deficit = "<< deficit << std::endl;
                        }
                    }
                ...
                remSym -= addSym;
                std::cout << "[1406] nFlowDL -> " << nFlowsDl << " remSym-> " << remSym << " nSymPerFlow0-> "  << nSymPerFlow0
                            << " nRemSymPerFlow (remSym / nFlowsTot) -> " << nRemSymPerFlow<<
    ...
    itUeInfo = itUeInfoStart;
    std::cout << "line[1483] symIdx-> "<<symIdx << std::endl;
    ...
    do
    {
        ...
        if(ueSchedInfo.m_dlSymbols > 0){
            ...
            std::cout << "[1495]itueinfo->first: "<< itUeInfo->first << " counter: "<< (int) sedighi << std::endl;
            td ++; // Total number of UEs requesting the symbol
    ...
    std::vector<double> final_sym;
    
    for (int y = 0; y < td; ++y){
        int fs = 0;

        std::cout << "Enter numbers (end with non-number or Ctrl+D): ";
        std::cin >> percentage;
        P_vec.push_back(percentage);

        fs = int(sedighi * percentage/100);
        ...
        final_sym.push_back(fs);
        
    }
    if(emad > 0 && !final_sym.empty()){
        auto max_it = std::max_element(final_sym.begin(), final_sym.end());
        *max_it += 1; // Add 1 to the max element
    }
    ...
    do
    {
        ...
        if(ueSchedInfo.m_dlSymbols > 0){
            ...
            // std::cin >> percentage;
            // ueSchedInfo.m_dlSymbols = int((sedighi) * P_vec[i] /100);
            // emad = emad - ueSchedInfo.m_dlSymbols;
            // if (emad < 0){
            // ueSchedInfo.m_dlSymbols += emad;
            // }
            ueSchedInfo.m_dlSymbols = final_sym[i];
        }
        i ++;
        itUeInfo ++;
        if (itUeInfo == ueInfo.end())
    ...
    {
        UeSchedInfo& ueSchedInfo = itUeInfo->second;
        std::cout << "line[1507] ueSchedInfo.m_dlSymbols =  "<< (int)ueSchedInfo.m_dlSymbols << std::endl;
        if (ueSchedInfo.m_dlSymbols > 0)
        {
            ...
            std::cout << "line [1524] dci.m_rnti-> "<< dci.m_rnti << " dci.m_format " << (int)dci.m_format <<
