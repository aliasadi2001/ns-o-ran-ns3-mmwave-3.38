    ...
    const struct MmWaveMacSchedSapProvider::SchedTriggerReqParameters& params)
{
    ...
    if (nFlowsTot > 0)
    {
        ...
        while (remSym > 0 && allocated)
        {
            ...
            while (remSym > 0)
            {
                ...
                // addSym =1 ;
                // int emad;
                // std::cout << "Enter addSym: "<< addSym <<"UE" << itUeInfo->first << std::endl;
                // std::cin >> emad ;
                // addSym = emad;
                itUeInfo->second.m_dlSymbols += addSym;
                remSym -= addSym;
                std::cout << "UE" << itUeInfo->first << " addSym: " << addSym << " remSym: " << remSym << std::endl;
                NS_ASSERT(remSym >= 0);
    ...
    do
    {
        ...
        if (ueSchedInfo.m_dlSymbols > 0)
        {
            ...
            std::cout << "UE" << itUeInfo->first << " symIdx: " << symIdx <<"dl-syms"<< ueSchedInfo.m_dlSymbols << std::endl;
            std::cin >> emad;
            std::cout << "UE" << itUeInfo->first << " emad: " << emad << std::endl;
            std::cin >> sedighi;
            ...
            NS_LOG_DEBUG("UE" << dci.m_rnti << " gets DL OFDM symbols " << +dci.m_symStart << "-"
                              ...
                              << +ret.m_sfnSf.m_sfNum << " slot " << +ret.m_sfnSf.m_slotNum);
            std::cout << "UE" << dci.m_rnti << " gets DL OFDM symbols " << +dci.m_symStart << "-"
                      << +(dci.m_symStart + dci.m_numSym - 1) << " tbs " << dci.m_tbSize
                      << " mcs " << +dci.m_mcs << " harqId " << +dci.m_harqProcess << " rv "
                      << +dci.m_rv << " in frame " << ret.m_sfnSf.m_frameNum << " subframe "
