     ...
     const struct MmWaveMacSchedSapProvider::SchedTriggerReqParameters& params)
 {
    ...
    //  int y = 0;
     do{
         ...
         if(ueSchedInfo.m_dlSymbols > 0){
            std::string logFilePath = "src/mmwave/model/es.log"; // Your log file path
            std::string searchKey = "UE";   // Key to search for
            double extractedValue = 0;   // Variable to store the found value
            bool valueFound = false;    // Flag to track if value was found

            //Step1 : open the log file
            ...
            if (logFile.is_open()) {
                ...
                {
                    if (line.find("UE :") != std::string::npos){
                        size_t colonPos = line.find(":");
                        if (colonPos != std::string::npos){
                            std::string numberPart = line.substr(colonPos + 1);

                            size_t firstDigit = numberPart.find_first_of("0123456789");
                            size_t lastDigit = numberPart.find_last_of("0123456789");
                            
                            if(firstDigit != std::string::npos && lastDigit != std::string::npos){
                                std::string numberStr = numberPart.substr(firstDigit, lastDigit - firstDigit +1);

                                try{
                                    ueNumber = std::stoi(numberStr);
                                    break;
                                } catch(...){
                                    std::cerr << "error in transform digit"<<std::endl;
                                }
                            }
                        }
                    }
            ...
            if (ueNumber != -1){
                std::cout <<"1 extract ue number" << ueNumber<< std::endl;
            }
    
            ueSchedInfo.m_dlSymbols = ueNumber;
            //  ueSchedInfo.m_dlSymbols = ceil((sedighi-counter) * random_list[y] / 100)+1;
            //  ueSchedInfo.m_dlSymbols = ceil((sedighi) * random_list[y] / 100);
            //  y++;
